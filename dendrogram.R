...
#args[1] is filepath to output folder, args[2] is distance score column name e.g. DistanceScore_d9

myreport<-read.table(gsubfn('%1', list('%1'=args[1]),'%1/output/alldistancestats.tsv'), header = TRUE, sep='\t')
writefilepath=gsubfn('%1|%2', list('%1'=args[1],'%2'=args[2]), '%1/output/dendrogram_%2.pdf')
...
plasmidscorecols<-c(which(colnamesmyreport=='Sample2'),which(colnamesmyreport==args[2]))
...
dendfilepath=gsubfn('%1|%2', list('%1'=args[1],'%2'=args[2]), '%1/output/dendrogram_%2.rd')
