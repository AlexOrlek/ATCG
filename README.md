...
The below table shows the most important outputs from running the pipeline with `--keep 2`, and using the `-s` input flag. Similar outputs are produced using `-s1` and `-s2` input flags, but trees will not be produced.
