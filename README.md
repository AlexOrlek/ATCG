...
* [1. Distance metrics](#distance-metrics)
    ...
    * [1.6 Examples](#16-examples)
        * [1.6.1 Toy example](#161-toy-example)
        * [1.6.2 Resistance plasmid comparison](#162-resistance-plasmid-comparison)
    * [1.7 FAQ](#17-faq)
* [2. Visualisation](#visualisation)
    ...
    * [2.6 Examples](#26-examples)
        * [2.6.1 Resistance plasmid comparison](#261-resistance-plasmid-comparison)
    * [2.7 FAQ](#27-faq)
* [License](#license)
...
* Complete genomes (e.g. bacterial plasmids; small collections of bacterial genomes)
* Incomplete genome sequences (or a combination of complete/incomplete sequences). Incomplete sequences (contigs or scaffolds) must be affiliated to a known genome and this affiliation is indicated in the sequence's FASTA header (see section [1.1 Input](#11-input))
...
* Analyse many large genomes; alignment-based approaches are too time-consuming, unless used in conjunction with faster approaches - faster, locus-based typing methods such as [MLST](https://pubmlst.org/general.shtml) or alignment-free comparative genomic tools such as [Mash](https://mash.readthedocs.io/en/latest/) could be used initially, to identify a subset of more closely related genomes, which could then be further analysed using ATCG.
...
* Conduct in-depth analysis of differences between two genomes (e.g. SNP analysis, categorisation of different types of structural rearrangement). Instead, use [NucDiff](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5508607/) or similar tool.
...
Alternatively, if all-vs-all comparison is not required, 2 input (multi-)FASTA files can be provided using flags `-1` and `-2`; pairwise comparisons will be conducted between but not amongst sequence(s) in each file; pairwise distances will be recorded but a tree will not be generated since distances are not available for all pairwise combinations. The order in which the FASTA files are provided to the -1/-2 flags will not affect results, however the sequences in the two files must be non-overlapping i.e. no sequence should appear in both files (ATCG will check this based on examination of the FASTA header identifiers).

`distance.py -1 query.fasta -2 genomes.fasta -o output-directory`
...
2. Where alignment ranges overlap at the same region on the query genome or the subject genome, the lower-scoring (based on bitscore, by default) overlapping alignment is trimmed to eliminate the overlap. This results in a set of "disjoint alignments".
3. Trimming performed on the query/subject genome is applied to the corresponding range on the subject/query genome, resulting in "trimmed alignments". 
    ...
    * If an alignment is trimmed from the same flank on both query and subject genome, then a maximal trim is applied i.e. if the alignment trim lengths differ between genomes, then the shorter trim will be extended to match the longer trim (see the [Toy example](#161-toy-example) below).
4. Calculation of distance metrics: Prior to trimming, the alignment coverage breadth on each genome is calculated. After alignment trimming, the trimmed alignments are used to calculate distance scores; breakpoint distances and alignment length statistics can optionally be calculated using trimmed alignments. A description of the statistics produced by ATCG, and formulae for their calculation are given [here](misc/statistics_calculation.pdf).
5. If all-vs-all BLAST was run, then tree(s) are built according to specified distance score(s).
...
The `-r` flag determines the parameter used to select the best alignment among overlapping alignments (bitscore, length, or percent identity).<br>
The `-l` flag specifies a length filter which is applied to trimmed alignments prior to calculating breakpoint distance and alignment length statistics. The rationale for the flag is as follows: even if alignment length is selected as the best hit selection criterion (`-r alnlen`), shorter alignments may not nest within the longest alignment if alignment positions are staggered; therefore, short remnants of trimmed alignments (the flanks that did not overlap the best alignment) may remain post-trimming. Excluding these artifactual remnant alignments whilst retaining longer (e.g. at least gene-sized) alignments should reduce bias when calculating breakpoint distance and alignment length statistics<br>
...
If you only want to obtain pairwise blast output, not distance statistics the `--blastonly` flag should be provided.<br>
The `--keep` flag is used to specify how much output to retain.<br>
...
To better understand the calculation of the various statistics, it may help to take a look at the [toy example](#161-toy-example) below.
...
The below table shows the most important outputs from running the pipeline with `--keep 2`, and using the `-s` input flag. Similar outputs are produced using `-1` and `-2` input flags, but trees will not be produced.
...
<br>
Notice how the red alignment is involved in alignment trimming: it overlaps with the blue alignment on sequence B and since the red alignment is longer, the blue alignment is trimmed. The red alignment also overlaps with the green alignment on sequence A; in this case, it is the shorter of the two alignments, so it is trimmed on sequence A. However, in the second step of trimming, because it is a '-' strand alignment, it is trimmed from the 3' end on sequence B.<br>
...
For simplicity, for the percent identity calculation, the numerator and denominator are multiplied by 2. This is because, in this example, the number of identical basepairs (numerator) and the alignment length (denominator) are the same in both BLAST directions. However, BLAST is not always symmetric; this is the rationale for aggregating across both BLAST directions. 


#### Additional toy example: contig input

An additional toy example is provided in the toy-example-contig-input directory, to demonstrate handling of input sequences comprising contigs. The example can be run by typing `Rscript granges_example.R` from within the toy-example-contig-input directory.

In this example, genomes A and B each comprise 3 contig sequences: A|1;A|2;A|3 and B|1;B|2;B|3, respectively. For simplicity, contig A|1 aligns to contig B|1 and so forth; in addition, each contig from genome A matches sequence A from the main toy example, and each contig from genome B matches sequence B of the main toy example. Therefore, as expected, the distance scores and breakpoint distance metrics calculated in toy-example-contig-input match those from the toy-example.

### 1.6.2 Resistance plasmid comparison
...
`distance.py -s sequences/plasmids.fasta -o atcgout --breakpoint --alnlenstats --trimmedalignments`
...
Overlapping alignments may reflect a scenario where mutliple duplicate stretches of repeat sequence from one genome map to a single homologous stretch in the other genome (as in the orange and pink alignments in the [toy example](#161-toy-example) above). Trimming overlapping alignments therefore reduces bias from repetitive sequences. [Previous authors](https://academic.oup.com/bioinformatics/article/21/10/2329/205556) have shown that trimming overlapping alignments improves distance-based species delimitation.
* **What happens if there are no BLAST alignments between two genomes?**
Pairwise comparisons yielding no BLAST alignments will not be included in the output. Genome(s) with no BLAST alignments to any other genome(s) will be recorded in the excluded.txt file. If no blast alignments are produced from any of the pairwise comparisons, the pipeline will be terminated with a warning message: "Warning: no blast alignments produced from genome comparison(s); pipeline terminated".
...
Yes, ATCG can be used to compare small collections of eukaryotic genomes. If a genome comprises multiple chromosomes but you wish to compare at the genome level, chromosomes or chromosomal contigs can be affiliated to genomes using the input format described above e.g. `genomeX|chromosomeN`. However, ATCG breakpoint distance is currently calculated with only prokaryotic genomes in mind. To accommodate eukaryotic genomes, where gene duplication is more common, the breakpoint definition would need to be adapted; for eukaryotic comparisons, pairs of genes that are in approximately, but not exactly, the same order would not be counted as a breakpoint (see [Rocha 2005](https://academic.oup.com/mbe/article/23/3/513/1110176)).   
...
Note that the trimmed alignment file(s) and sequence length file generated by the `distance.py` script can be used as input, and in this case the `-s` flag syntax would be trimmedalignments_GENOMENAME.tsv. The original (un-trimmed) BLAST alignment files can also be used as input and in this case the `-s` flag syntax would be GENOMENAME/alignments.tsv<br>
...
`--sequencefills` specifies the fill colour(s) of sequences. If a genome comprises multiple contigs, then comma-separated colours can be used to indicate colours of adjacent contig sequences. Space-separated colour terms indicate colour(s) per genome. If there are more contig sequences/genomes than comma-separated colours/space-separated colour terms, then the colour values will be recycled so that each sequence is assigned a colour. For example, `white` will colour all sequences white; `lightgray,darkgray lightblue,darkblue` will colour sequences in the first genome grayscale and sequences in the second genome blue shades. The `--sequenceoutlines` and `--sequencetypes` flags work in a similar way; they specify sequence outline colour(s) and sequence symbols (`lines` or `blocks`), respectively.<br> 
`--positivecols` and `--negativecols` define a colour scale gradient reflecting percent identity of positive and negative orientation alignments, respectively. At least two arguments must be provided; the left-hand-side argument defines the colour for higher identity alignments, and vice-versa. If `red red` is provided then all alignments will be coloured red regardless of percent identity. On the other hand, a rainbow colour gradient could be defined (`red yellow green blue`). Note that colours will be made transparent to support visualisation of criss-crossing alignments, and this will make specified colours appear a slightly lighter shade.<br>
`--gffannotationtype` specifies which annotation types to include. Annotation type is given in the 3rd column of a gff3 file; by default only coding sequences (CDS) are plotted.<br>
`--annotationtxt_type` specifies whether text annotations are plotted at the midpoint (`mid`) of an annotation, or whether a text marker spanning the length of the annotation is plotted (`spanning`).<br>
...
Below I demonstrate how - using input generated from code given [above](#162-resistance-plasmid-comparison), and a small number of visualisation commands - ATCG can generate a plot similar to the one shown in the paper by [Decraene _et al_. 2018](https://aac.asm.org/content/62/12/e01689-18.long) (Fig.4A).<br>

A gff3 annotation file (generated by running prokka on the sequences) is stored in the resistance-plasmid-example/annotations directory. First, if you haven't done so already, run the `distance.py` script, using code given [above](#162-resistance-plasmid-comparison). Then, navigate to the resistance-plasmid-example directory, and use the `getfeatureinput.py` script to split the prokka annotation file on a per-genome basis, while ensuring the gff3 is correctly formatted for ATCG visualisation:<br>
...
This depends on how large the sequences that you want visualise are but some tips are given below:
    ...
    * Note that setting `--comparisontype` to `singlereference` rather than `chain` affects more than just which comparisons are shown. When `chain` is provided, by default, gaps are introduced between contigs (as in pKPC-CAD2 from the Example) and preceding gaps may be introduced to optimise display (as in pKPC-CAD3 from the Example); when `singlereference` is provided, no gaps are permitted. This is due to constraints associated with the underlying genoPlotR software.
