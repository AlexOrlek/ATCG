...
input_group.add_argument('-s','--syntax', help='File name / file path syntax for identifying alignment files and (optionally) annotation files within the input/feature directories e.g. bestblastalignments_GENOMENAME.tsv GENOMENAME.gff (required)', nargs='+', required=True)
...
input_group.add_argument('-p', '--seg_plots', help='A list of seg_plot objects saved as an rds file (optional)' if show_all_help else argparse.SUPPRESS, required=False)
...
vis_group1.add_argument('--main_pos', help='Title text position if title text is provided (default: centre)' if show_all_help else argparse.SUPPRESS, default='centre', choices=['centre','left','right'], type=str)
vis_group1.add_argument('--legend_orientation', help='Orientation of the alignment colour scale legend (default: vertical)' if show_all_help else argparse.SUPPRESS, default='vertical', choices=['vertical','horizontal'], type=str)
vis_group1.add_argument('--legend_textcex', help='Relative size of the legend text (default: 0.5)' if show_all_help else argparse.SUPPRESS, default='0.5', type=positivefloat)
vis_group1.add_argument('--legend_titlecex', help='Relative size of the legend title text (default: 0.6)' if show_all_help else argparse.SUPPRESS, default='0.6', type=positivefloat)
vis_group1.add_argument('--positivecols', help='Colours defining a colour scale reflecting percent identity of positive orientation alignments e.g. red yellow would plot high identity alignments red and low identity alignments yellow; more than 2 colours can be provided e.g. red yellow green blue (default: "#9e0d0d" "#eacaca")', nargs='+',default=['#9e0d0d','#eacaca'],type=str) #colour on left is darkest and represents higher identity alignments (need to reverse vector in R for this to be the case).
vis_group1.add_argument('--negativecols', help='As above but for negative orientation alignments (default: "#0000b7" "#c7c7ef")', nargs='+',default=['#0000b7','#c7c7ef'],type=str)
...
vis_group1.add_argument('--tree_width', help='Width of the tree area as a proportion of total plot width (default: 0.2)' if show_all_help else argparse.SUPPRESS, default='0.2', type=positivefloat)
vis_group1.add_argument('--tree_branch_labels_cex', help='Relative size of the tree branch labels, if present (default: 0 i.e. not plotted)' if show_all_help else argparse.SUPPRESS, default='0', type=positivefloat)
vis_group1.add_argument('--tree_scale', action='store_true', help='If flag is provided, a scale will be plotted for the tree (default: no tree scale)' if show_all_help else argparse.SUPPRESS)
vis_group1.add_argument('--seg_plot_height', help='The height of the seg_plot regions as a proportion of the comparison region (default: 0.3)' if show_all_help else argparse.SUPPRESS, default='0.3', type=positivefloat)
...
vis_group1.add_argument('--seg_plot_yaxis', help='Can be NULL, FALSE or numeric. In the first two cases, no y-axis is drawn for the seg_plots. If numeric, a y axis is drawn with approximately that number of ticks (default: NULL)' if show_all_help else argparse.SUPPRESS, default='NULL', type=segplotyaxis)
vis_group1.add_argument('--seg_plot_yaxis_cex', help='Relative size of the seg_plot_yaxis text (default: 0.5)' if show_all_help else argparse.SUPPRESS, default='0.5', type=positivefloat)
...
vis_group2.add_argument('--sequenceoutlines', help='Outline colour(s) of sequence symbols; takes comma-/space-separated input to map outline colours to sequences (similar to --sequencefills flag) (default: black)', nargs='+',default=['black'],type=str)
vis_group2.add_argument('--sequencetypes', help='Symbol type(s) of sequence symbols (either "blocks" or "lines"); takes comma-/space-separated input to map symbol types to sequences (similar to --sequencefills flag) (default: blocks)', nargs='+',default=['blocks'], choices=['blocks','lines'], type=str)
...
vis_group2.add_argument('--dna_seg_label_cex', help='The relative size of dna_seg_labels text (default: 0.8)' if show_all_help else argparse.SUPPRESS, default='0.8',type=positivefloat)
vis_group2.add_argument('--dna_seg_label_col', help='The colour of dna_seg_labels text; provide either a single value or a space-separated list of the same length as the number of genomes (default: black)' if show_all_help else argparse.SUPPRESS, default=['black'],nargs='+',type=str)
...
vis_group2.add_argument('--minimum_gap_size', help='The space between subsequences, expressed as the proportion of plotting width taken up by gaps (default: 0.05)' if show_all_help else argparse.SUPPRESS, default='0.05',type=positivefloat)
vis_group2.add_argument('--scale', help='Should a global scale be shown at the bottom of the plot; provide true/false (case-insensitive) (default: false)' if show_all_help else argparse.SUPPRESS, default='false',choices=['true','True','TRUE','false','False','FALSE'],type=str)
vis_group2.add_argument('--dna_seg_scale', help='Should per-genome scales be shown; provide true/false (case-insensitive) (default: true)' if show_all_help else argparse.SUPPRESS, default='true',choices=['true','True','TRUE','false','False','FALSE'],type=str)
vis_group2.add_argument('--dna_seg_scale_cex', help='The relative size of per-genome scales (default: 0.7)' if show_all_help else argparse.SUPPRESS, default='0.7',type=positivefloat)
vis_group2.add_argument('--dna_seg_scale_n_ticks', help='The approximate number of ticks on the scale of the longest genome (default: 7)' if show_all_help else argparse.SUPPRESS, default='7',type=positiveint)
...
vis_group3.add_argument('--annotation_lty', help='Specifies the default outline line type of the annotation symbol (default: 1)' if show_all_help else argparse.SUPPRESS, default='1',type=ltyint)
vis_group3.add_argument('--annotation_lwd', help='Specifies the default outline line width of the annotation symbol (default: 0.5)' if show_all_help else argparse.SUPPRESS, default='0.5',type=positivefloat)
...
vis_group3.add_argument('--annotationtxt_type', help='Specifies whether annotation text positioned in the middle of the annotation range or spanning the annotation range (default: mid)' if show_all_help else argparse.SUPPRESS, default='mid', choices=['mid','spanning'], type=str)
vis_group3.add_argument('--annotationtxt_height', help='Specifies how much space to allocate to annotation text; calculated automatically based on annotation text length unless specified' if show_all_help else argparse.SUPPRESS, type=positivefloat)
vis_group3.add_argument('--annotationtxt_rot', help='Specifies the annotation text rotation (default: 40)', default='40',type=int)
vis_group3.add_argument('--annotationtxt_cex', help='Specifies the relative size of annotation text (default: 0.7)' if show_all_help else argparse.SUPPRESS, default='0.7',type=positivefloat)
...
vis_group3.add_argument('--annotationtxt_exclusion_file', help='Exclusion criteria for text annotations, text file (optional)' if show_all_help else argparse.SUPPRESS)
vis_group3.add_argument('--annotationtxt_inclusion_file', help='Inclusion criteria for text annotations, text file (optional)' if show_all_help else argparse.SUPPRESS)
vis_group3.add_argument('--annotationtxt_exclinc_casesensitive', action='store_true', help='If flag is provided, exclusion/inclusion criteria will be case-sensitive when matching annotations (default: case-insensitive)' if show_all_help else argparse.SUPPRESS)
